...
string[,] matrix = {{" "," "," "," "," "},
                    ...
                    {" "," "," ","$"," "},
                    {" "," "," "," "," "}};
Dictionary<int, string[,]> Lvls =  new Dictionary<int, string[,]>
{
    {1, new string[,]
        {
            {" "," "," "," "," "},
...
int SelecLvlGame()
{
    ...
    {
       Console.WriteLine("Уровень "+ item.Key);
    }
...
void Game()
{
    while(true)
    {
        ...
        matrix[y,x] = " ";
        ConsoleKeyInfo User_keyTab = Console.ReadKey();
        if (User_keyTab.Key == ConsoleKey.E) {
            SelectMenuPlayer();
        }

        if(User_keyTab.Key == ConsoleKey.W) {
            if(Barrier(x,y-1)) {
                y--;
            }
        }
        if(User_keyTab.Key == ConsoleKey.S) {
            if(Barrier(x,y+1)) {
                y++;
            }
        }

        if(User_keyTab.Key == ConsoleKey.A) {
            if(Barrier(x-1,y)) {
                x--;
            }
        }

        if(User_keyTab.Key == ConsoleKey.D) {
            if(Barrier(x+1,y)) {
                x++;
            }
        }
